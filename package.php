        ...
        <div id="side-panel-pusher">
            <div id="main-container">
                <div id="page-wrap">
                    ...
                    <?php
                    $ppq = mysql_query("select * from package_CMS_page_title");
                    $ppv = mysql_fetch_array($ppq);
                    ?>
                    <div id="titlebar" class="titlebar-hautq cover">
                        <div class="container relative">
                            <div id="titlebar-text">
                                <h2 id="titlebar-title"><strong> <?=$ppv['heading']?></strong></h2>
                            </div>
                    ...
                    <div id="page-content" class="no-sidebar-layout"><div class="container"><div id="the-content" >
                                ...
                                <div class="ui--title ui--animation ui--title-bordered text-left">
                                    <div class="ui--title-holder">
                                        <h3 class="ui--title-text"><strong><?=$ppv['title']?></strong></h3>
                                ...
                                <div  class="ui--animation-in make--fx--appear ui--pass clearfix" data-fx="fx--appear" data-delay="150" data-start-delay="0">
                                    ...
                                    <p><?=$ppv['subtitle']?></p>
                                    <?php
                                    $pq = mysql_query("select * from package ORDER BY package_id DESC");
                                    ?>
                                    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="pro-table">
                                        <tr>
                                            ...
                                            <?php
                                            $p=0;
                                            while($pqd = mysql_fetch_array($pq))
                                            {
                                                $p++;
                                                if($p==1){
                                                    $class = 'th-col5';
                                            ...
                                            ?>
                                            <th width="20%" class="<?=$class;?>"><?=$pqd['package_name'];?> <br> <?='$'.$pqd['total_price'];?></th>
<!--                                            <th width="20%" class="th-col2">MID-LEVEL CEO <br>
                                        ...
                                        <?php
                                        $pfq = mysql_query("select * from package_feature");
                                        while($pfqt = mysql_fetch_array($pfq)){
                                        ?>         
                                            <tr>
                                                <td><?=$pfqt['name']?></td>
                                                <?php
                                                $pq=mysql_query("select package_id from package ORDER BY package_id DESC");
                                                while($pqt = mysql_fetch_array($pq))
                                                {
                                                    //echo "select * from package_CMS_table WHERE package_id='".$pqt['package_id']."' AND feature_id='".$pfqt['id']."'";
                                                    $cmstq = mysql_query("select * from package_CMS_table WHERE package_id='".$pqt['package_id']."' AND feature_id='".$pfqt['id']."'");
                                                    $cmsd = mysql_fetch_assoc($cmstq);
                                                ?>
                                                <td style="text-align:center">
                                                    <?php if($cmsd['value']=='0'){ ?>
                                                        <i class="ui--icon fontawesome-remove icon-inline-block" style="font-size: 30px;  width: 22px;  height: 22px;  color: #ed888d !important;  margin-right: 5px;"></i>
                                                    <?php }else if($cmsd['value']=='1'){  ?>
                                                        <i class="ui--icon fontawesome-ok icon-inline-block" style="font-size: 30px;  width: 22px;  height: 22px;  color: #169fe6 !important;  margin-right: 5px;"></i>
                                                    <?php } else {
                                                        echo $cmsd['value'];
                                                        }
                                                    ?>
                                        ...
                                        <tr>
                                            <td>&nbsp;</td>
                                            <td style="text-align:center"><a href="register.php" class="btn btn-large btn-block btn-icon-left btn-yellow">Signup Now</a></td>
                                            <td style="text-align:center"><a href="register.php" class="btn btn-large btn-block btn-icon-left btn-yellow">Signup Now</a></td>
                                            <td style="text-align:center"><a href="register.php" class="btn btn-large btn-block btn-icon-left btn-yellow">Signup Now</a></td>
                                        </tr>
...
<div class="ui--tabs ui--tabs-mini ui--tabs-mini-horizontal clearfix text-left ui--done">
    <div class="ui--tabs-header clearfix">
        <div class="ui--tabs-header-holder">
            <div style="left: -4000px;" class="ui--tabs-border-before"></div>
            <div style="left: 99px;" class="ui--tabs-border-after"></div>
            <ul class="ui--tabs-titles clearfix unstyled">
                <?php
                $ptq = mysql_query("select * from package_CMS_tab");
                $t=0;
                $ptqtotle = mysql_num_rows($ptq);
                while($ptqt = mysql_fetch_array($ptq)){
                 $t++;
                 $active = $t==1?'active':'';
                 $last_item = ($t==1 ? 'first-item': ($t==$ptqtotle ? 'last-item':''));
                ?> 
                <li class="<?=$last_item.' '.$active;?>"><h5><a href="#faq_<?=$ptqt['id']?>"><?=$ptqt['title']?></a></h5></li>
                <?php
                }
                ?>
            </ul>
    ...
    <ul class="ui--tabs-contents text-left clearfix">
        <?php
        $ptqv = mysql_query("select * from package_CMS_tab");
        $tv=0;
        while($ptqtv = mysql_fetch_array($ptqv)){
         $tv++;
         $active = $tv==1?'first-item active':'hidden';
        ?> 
        <li class="<?=$active?>"> 
            <div class="ui-row row">

                <div class="ui-column span12"> 
                    <h3><strong> <?=$ptqtv['title']?></strong></h3>
                    ...
                    <div class="ui-row row">
                        <div class="ui-column span12">
                            <?php
                            $ptqc = mysql_query("select * from package_CMS_tab_content WHERE tab_id='".$ptqtv['id']."'");
                            $q=0;
                            while($ptqtc = mysql_fetch_array($ptqc)){
                            $q++;    
                            ?> 
                            <div delay="0" data-group="toggle-group1" style="margin-bottom: 1px;" class="ui--toggle clearfix ui--toggle-state-closed fx--appear ui--animation-fire">
                                <div class="ui--toggle-title">
                                    <a href="javascript:;">
                                        <span class="ui--toggle-icon"><i class="ui--icon show-when-closed fontawesome-plus"></i><i class="ui--icon show-when-opened fontawesome-minus"></i></span>
                                        <span class="ui--toggle-title-text heading"><?=$q;?>. <?=$ptqtc['question']?></span>
                                    </a>
                                </div>
                                <div class="ui--toggle-content">
                                    <div delay="0" class="auto-format fx--appear ui--animation-fire">
                                        <p><?=$ptqtc['value']?></p>
                                    </div>
                                </div>
                            </div>
        ...
        ?>
                                            ...
                                            </div></div><div  class="ui--toggle ui--animation clearfix ui--toggle-state-closed" style="margin-bottom: 1px;" data-group="toggle-group1">
