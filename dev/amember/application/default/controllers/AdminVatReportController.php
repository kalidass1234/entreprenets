...
class AdminVatReportController extends Am_Controller
{
    ...
    function getMainDataSource()
    {
        ...
        $ds->leftJoin('?_user', 'u', 't.user_id = u.user_id')
            ...
            ->addField('concat(if(country is null, "", country), "-", if(invoice.tax_rate is null, "", invoice.tax_rate))', 'country_rate');
...
class Am_Grid_EU_Vat_Report_Details extends Am_Grid_Editable
{
    ...
    function renderTitle($noTags = false)
    {
        ...
        {
            $msgs[] = ___('Country: %s', Am_Controller::escape(implode(", ", (array)$country)));
        }

        if ($paysys_id = @$filter['paysys_id'])
        {
            $msgs[] = ___('Paysystem: %s', Am_Controller::escape($paysys_id));
        }
...
class Am_Grid_Action_EU_VAT_RowDetails extends Am_Grid_Action_Abstract
{
    ...
    function getUrl($record = null, $id = null)
    {
        ...
        $params = array(
            ...
            'return_url'=>  Am_Controller::getFullUrl()
            );
        if(!is_null($record))
        {
...
class Am_Grid_Filter_EU_VAT_Report extends Am_Grid_Filter_EU_VAT
{
        ...
        function renderInputs()
    {
        ...
        $countryOptions = Am_Controller::renderOptions(
