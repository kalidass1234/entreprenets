...
class Am_Newsletter_Plugin_Autoresponderpro extends Am_Newsletter_Plugin
{
    ...
    {
        $form->addText('username', array('size' => 60))
            ->setLabel(array('Username','The user name used to login to the AutoResponderPro Email Marketer'))
            ->addRule('required');
        
        $form->addText('usertoken', array('size' => 60))
            ->setLabel(array('Token','The unique token assigned to the user account used above'))
            ->addRule('required');
    ...
    private function getUserCustomFields(User $user)
    {
