...
class Am_Newsletter_Plugin_Campaignmonitor extends Am_Newsletter_Plugin
{
    ...
    public function _initSetupForm(Am_Form_Setup $form)
    {
        $form->addText('api_key', array('size' => 40))
            ->setLabel(array('Campaignmonitor API Key', 'from your campaignmonitor account -> Account Settings -> API Key'))
            ->addRule('required');
        ...
        if($this->getConfig('api_key'))
        {
            ...
            $el = $form->addSelect('client_api_id')
                ->setLabel(array('Client API Id', 'select your client which you will use with aMember'))
            ->loadOptions($options);
            if($options)
                $el->addRule('required');
        }
        ...
        $form->addTextarea('custom_fields', array('rows' => 5, 'cols' => 20))
            ->setLabel(array('Additional Fields', "campaignmonitor_field|amember_field\n"
                . "eg: FNAME|name_f\n"
                . "USERIP|remote_addr\n"
                . "ADDED|added\n"
                . "one link - one string\n"
        ...
        $form->addAdvCheckbox('debug_mode')
            ->setLabel(array('Debug Mode Enabled', "write debug info to logs\nit's recommended enable it at the first time"));
    }
    ...
    public function updateWebhooks($vars)
    {
        ...
        foreach ($listIds as $listId)
        {
            ...
            $result = $wrap->create_webhook(array(
                ...
                'Url' => ROOT_SURL . '/misc/misc-' . $this->getId(),
