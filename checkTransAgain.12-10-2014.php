...
{
	 //echo "select amount from final_e_wallet where user_id='".$_SESSION['ref_id']."'";
	 
	 $selectSponserAmount=mysql_query("select amount from final_e_wallet where user_id='".$_SESSION['ref_id']."'");
	 $fetchuserFetch=mysql_fetch_array($selectSponserAmount);
	 $totalSponserAmount = $fetchuserFetch['amount'];
	 $_SESSION['totalSponserAmount'] = $totalSponserAmount;
	 

	 $usertcode = $_REQUEST['transaction_pin'];
 	 $_SESSION['transaction_pins'] = $usertcode; 	
	 $selectuser12=mysql_query("select t_code from registration where t_code='$usertcode' and user_pass = '".$_REQUEST['pass']."'");
	 $fetchuser12=mysql_fetch_array($selectuser12);
	 $fetchuser1Count2 = mysql_num_rows($selectuser12);

	$_SESSION['payment_mode'] = 'ewallets';
	if($fetchuser1Count2 > 0)
	{
		if($_SESSION['Pprice']<=$totalSponserAmount)
		{
		...
		{
			$msg = "Sorry, you have amount less than INR ".$_SESSION['Pprice'].". You can not upgrade your plan.";
		}
	}
	else
	{
		$msg = "Please check your Details. Your details are wrong.";
	}
					...
					//alert(html);					 
					 if(html=='yes'){
						 ...
						 $("#Password1").removeClass("ajaxdiv");
						  $('#sponser_ajax1').html("This is invalid Cash Code."); 	
							$('#Password1').focus();
...
<div class="row eternity-form">
 ...
 <font size="4px;" color="red"> <?php echo $msg; ?></font>
              <header>E-Wallet form</header>

              <fieldset>
                            <p>Please fill Transaction Password below</p>
                <section>
                  <label class="input"> <i class="icon-append icon-user"></i>
                    <input type="text" placeholder="Transaction Password" name="transaction_pin" value="" required>
                ...
                <section>
                  <label class="input"> <i class="icon-append icon-user"></i>
                    <input type="text" placeholder="Password" name="pass" value="" required>
                    <b class="tooltip tooltip-bottom-right">Only latin characters and numbers</b> </label>
                    <div id="ajax_username" style="font-size:16px; color:red; font-weight:bold;"></div> 
                </section>
