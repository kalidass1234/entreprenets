        ...
        <div class="widget">
          ...
          <div class="widget-content">
			...
			if(isset($_REQUEST['search']))
			{
				...
				$url='admin_main.php?page_number=173&'.$query_string;
			}
			else
			{
				$url='admin_main.php?page_number=173&1=1'.$search_string;
			}
			...
			if($per_page != "all"){
			  ...
			  $res_products_tol=$obj_query->query("id","add_money","status=0 $search_string $con_search order by id desc ");
			  $total_row=$obj_query->num_row($res_products_tol);
			  $pages = ceil($total_row/$per_page);
						 
			  $res_cat=$obj_query->query("*","add_money","status=0 $search_string $con_search order by id desc $limit");
			...
			while($row_cat=$obj_query->get_all_row($res_cat))
			{
			  ?>
                <tr>
                  ...
                  <td><?php echo $row_cat['pay_date']; ?></td>
                  <td>&nbsp;
                  <span><a href="load_wallet_confirm.php?page_number=173&user_id=<?php echo $row_cat['user_id'];?>&id=<?php echo $row_cat['id'];?>&confirm=confirm" title="Confirm">Confirm</a></span></td>
                </tr>
                <?php 
				$s_no++;
			  } 
