  ...
  <div class="mainbar"> 
    ...
    <div class="page-head">
      ...
      <div class="bread-crumb "> <a href="index.php"><i class="fa fa-home"></i> Home</a> 
        <!-- Divider --> 
        <span class="divider">/</span> <a href="#" class="bread-current">Support Ticket</a> </div>
      <div class="clearfix"></div>
    ...
    <div class="matter">
      ...
      <div class="clearfix"></div>
        <div class="widget">
          <div class="widget-head">
            <div class="pull-left">Closed Ticket List</div>
            <div class="widget-icons pull-right"> <a href="#" class="wminimize"><i class="fa fa-chevron-up"></i></a> <a href="#" class="wsettings"><i class="fa fa-wrench"></i></a> <a href="#" class="wclose"><i class="fa fa-times"></i></a> </div>
            <div class="clearfix"></div>
          </div>
          <div class="widget-content">
          ...
          <input type="hidden" name="action" value="Close_Ticket">
			...
			if(isset($_REQUEST['search']))
			{
				...
				$url='admin_main.php?page_number=42&'.$query_string;
			}
			else
			{
				$url='admin_main.php?page_number=42&'.$search_string;
			}
			...
			if($per_page != "all"){
			 ...
			 if($admin_type=='super_admin')
			  ...
			  $res_products_tol=mysql_query("select * from tickets f inner join registration r on f.user_id=r.user_id where f.status=1 $country_search $search_string $con_search order by f.id desc") ;
			  
			 // $res_products_tol=$obj_query->query("id","tickets","status=1 $search_string $con_search order by id desc ");
			  $total_row=$obj_query->num_row($res_products_tol);
			  $pages = ceil($total_row/$per_page);
						 
			  $res_cat=mysql_query("select * from tickets f inner join registration r on f.user_id=r.user_id where f.status=1 $country_search $search_string $con_search order by f.id desc $limit");
			...
			while($row_cat=$obj_query->get_all_row($res_cat))
			{
			  ?>
                <tr>
                 ...
                 <td><?php echo $row_cat['response']; ?></td>
                  <td><?php echo $row_cat['c_t_date']; ?></td>
                </tr>
                <?php 
					$s_no++;
			  }
