...
if(isset($_REQUEST['search']) || $_REQUEST['user_id'])
{
	...
	if($id!='')
	{
		...
		if($stage=='1')
		{
			$quer2="select * from board_status where board_owner_id='$id' and package_id = '70.00' AND board_number= '$stage' AND upper_id = '$id' ORDER BY cell_no ASC LIMIT 1,3";
		...
		{
			$quer2="select * from board_status where board_owner_id='$id' and package_id = '70.00' AND board_number= '$stage' AND upper_id = '$id' ORDER BY cell_no ASC LIMIT 0,3";
			$data2=mysql_query($quer2);
			while($x2=mysql_fetch_array($data2))
			{
			$arr2[]=$x2;
...
function changehistorys(val,user_id){
	...
	if(val!='' && val!='<?=$_SESSION['changehistoryval'];?>'){
	  ...
	  if(con==true){
		document.location.href='admin_main.php?page_number=204&user_id='+user_id+'&stage='+val;
		return true;
	  } else {
		return false;
	  }
