            ...
            <div class="col-md-12">
              <div class="widget">
                <div class="widget-head">
                  <div class="pull-left">Income By BMC1 Detail </div>
                ...
                <div class="widget-content">
                  <div class="padd">
                    <form action="admin_main.php?page_number=178"  method="post" class="validate" id='form1'>
                     <fieldset>
...
<script>function coderHakan(){var sayfa = window.open('','','width=800,height=800');sayfa.document.open("text/html");sayfa.document.write(document.getElementById('printArea').innerHTML);sayfa.document.close();sayfa.print();
          ...
          <div class="widget">
                ...
                <div class="widget-head">
                  <div class="pull-left">Income By BMC1</div>
...
<div style="width:96%; " id="printArea">
			...
			if(isset($_REQUEST['search']))
			{
				...
				{
					$search_string.=" and l_date between '".$_REQUEST['from_date']."' and '".$_REQUEST['to_date']."'";
				}
			...
			if(isset($_REQUEST['search']))
			{
				...
				$url='admin_main.php?page_number=178&'.$query_string;
			}
			else
			{
				$url='admin_main.php?page_number=178&'.$search_string;
			}
			...
			if($per_page != "all"){
			  ...
			  $limit = " ";
			  //echo "select * from level_income b inner join registration r on b.income_id=r.user_id where b.income_id!='56789' AND b.package_id='14' $search_string $con_search order by b.l_id desc";
			  $res_products_tol=mysql_query("select * from level_income b inner join registration r on b.income_id=r.user_id where b.income_id!='56789' AND b.package_id='14' $search_string $con_search order by b.l_id desc");
			  $total_row=$obj_query->num_row($res_products_tol);
			  $pages = ceil($total_row/$per_page);
					
			  $res_cat=mysql_query("select income_id,commission, l_date,bonus_name from level_income where income_id!='56789' AND package_id='14' $search_string $con_search GROUP by income_id $limit");
			  ...
			  {
				$res_catb=mysql_fetch_array(mysql_query("select SUM(commission) as commissions from level_income where package_id='14' AND income_id!='56789' AND income_id='".$row_cat['income_id']."' "));

              ?>
                <tr>
                    <td><?php echo $s_no;?></td>
                    <td><?php echo $row_cat['l_date'];?></td>
                    <td><?php echo $row_cat['income_id'];?></td>
                    <td><?php echo $obj_query->get_field_name("registration","user_name","user_id='$row_cat[income_id]'");?></td>
					<td class="center"><?php echo $res_catb['commissions'];?></td>
					<td><?php echo $res_catb['commissions'];?></td>
				...
				?>
                    ...
                    <div class="export padding-space"><a href="exportBMC1.php">Export in excel</a></div>
