            ...
            <div class="col-md-12">
              <div class="widget">
                <div class="widget-head">
                  <div class="pull-left">Pending Order Detail </div>
                ...
                <div class="widget-content">
                  <div class="padd">
                    <form action="admin_main.php?page_number=20"  method="post" class="validate" id='form1'>
                      <fieldset>
        ...
        <div class="row">  <div class="printer"><img src="images/printer_icon.png"></div></div>
          <div class="widget">
			...
			if(isset($_REQUEST['search']))
			{
				...
				$url='admin_main.php?page_number=20&'.$query_string;
			}
			else
			{
				$url='admin_main.php?page_number=20&'.$search_string;
			}
			...
			if($per_page != "all"){
			  ...
			  $res_products_tol=mysql_query("select * from amount_detail a inner join registration r on a.user_id=r.user_id where a.status=0 $country_search $search_string $con_search order by a.am_id desc");
			  $total_row=$obj_query->num_row($res_products_tol);
			  $pages = ceil($total_row/$per_page);
				
			 // $res_cat=$obj_query->query("*","amount_detail"," status=0 $search_string $con_search order by am_id desc $limit");
			  
			 
			   $res_cat=mysql_query("select * from amount_detail a inner join registration r on a.user_id=r.user_id where a.status=0 $country_search $search_string $con_search order by a.am_id desc $limit");
