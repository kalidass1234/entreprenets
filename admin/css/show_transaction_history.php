        ...
        <div class="widget">
          ...
          <div class="widget-content">
			...
			if(isset($_REQUEST['search']))
			{
				...
				$url='show_transaction_history.php?'.$query_string;
			}
			else
			{
				$url='show_transaction_history.php?1=1'.$search_string;
			}
			...
			if($per_page != "all"){
			  ...
			  $res_products_tol=$obj_query->query("id","credit_debit","1=1 $search_string $con_search order by id desc ");
			  $total_row=$obj_query->num_row($res_products_tol);
			  $pages = ceil($total_row/$per_page);
						 
			  $res_cat=$obj_query->query("*","credit_debit"," 1=1 $search_string $con_search order by id desc $limit");
