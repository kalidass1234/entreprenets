        ...
        <div class="widget">
          ...
          <div class="widget-content">
			...
			if(isset($_REQUEST['search']))
			{
				...
				$url='admin_main.php?page_number=174&'.$query_string;
			}
			else
			{
				$url='admin_main.php?page_number=174&1=1'.$search_string;
			}
			...
			if($per_page != "all"){
			  ...
			  $res_products_tol=mysql_query("select * from finance_request f inner join registration r on f.user_id=r.user_id where f.status=0 $country_search $search_string $con_search order by f.id desc") ;
			  $total_row=$obj_query->num_row($res_products_tol);
			  $pages = ceil($total_row/$per_page);
						 
			  $res_cat=mysql_query("select * from finance_request f inner join registration r on f.user_id=r.user_id where f.status=0 $country_search $search_string $con_search order by f.id desc $limit");
			...
			while($row_cat=$obj_query->get_all_row($res_cat))
			{
			  ?>
                <tr>
                  ...
                  <td>&nbsp;
                  <span><a href="finacial_confirm.php?page_number=174&user_id=<?php echo $row_cat['user_id'];?>&id=<?php echo $row_cat['id'];?>&confirm=confirm" title="Confirm">Confirm</a></span></td>
                </tr>
                <?php 
				$s_no++;
			  } 
