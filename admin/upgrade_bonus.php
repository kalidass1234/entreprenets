            ...
            <div class="col-md-12">
              <div class="widget">
                <div class="widget-head">
                  <div class="pull-left">Sponsorship Bonus Detail </div>
...
<script>function coderHakan(){var sayfa = window.open('','','width=800,height=800');sayfa.document.open("text/html");sayfa.document.write(document.getElementById('printArea').innerHTML);sayfa.document.close();sayfa.print();
          ...
          <div class="widget">
                ...
                <div class="widget-head">
                  <div class="pull-left">Sponsorship Bonus</div>
...
//echo "select * from binary_income b inner join registration r on b.income_id=r.user_id where b.income_id!='cmp' $search_string $con_search order by b.id desc";
			  $res_products_tol=mysql_query("select * from binary_income b inner join registration r on b.income_id=r.user_id where b.income_id!='cmp' AND b.bonus_name='3' $search_string $con_search order by b.id desc");
			  $total_row=$obj_query->num_row($res_products_tol);
			  $pages = ceil($total_row/$per_page);

			  $res_cat=mysql_query("select * from binary_income b inner join registration r on b.income_id=r.user_id where b.income_id!='cmp' AND b.bonus_name='3' $search_string $con_search order by b.id desc $limit");
			  ...
			  {
				 $select = mysql_fetch_array(mysql_query("SELECT * FROM registration WHERE user_id='".$row_cat['purchaser_id']."'"));
				 $selectP = mysql_fetch_array(mysql_query("SELECT * FROM package WHERE package_id='".$row_cat['package_id']."'"));
              ?>
                <tr>
                    <td><?php echo $s_no;?></td>
                    ...
                    <td><?php echo $row_cat['purchaser_id'];?></td>
                    <td><?php echo $select['user_name'];?></td>
                    <td><?php echo $selectP['package_name']."-".$selectP['total_price'];?></td>
                    <td><?php echo $row_cat['commission']*10/100;?></td>
                    <td><?php echo $row_cat['commission']*5/100;?></td>
                    <td><?php echo $row_cat['commission']*5/100;?></td>
                    <td><?php echo $row_cat['commission'];?></td>

                    <td><?php echo $row_cat['final_amount'];?></td>
				...
				?>
                    ...
                    <div class="export padding-space"><a href="exportSponserShip.php">Export in excel</a></div>
