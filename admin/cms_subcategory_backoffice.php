...
<div class="content"> 
  ...
  <div class="mainbar"> 
    ...
    <div class="matter">
      ...
      <a  style="float:left" href="<?php echo SITE_URL; ?>admin/admin_main.php?page_number=169"> <button class="btn btn-danger side"  type="button" id="button" >Add Sub Category</button></a>&nbsp;&nbsp;
        ...
        <div class="widget">
          ...
          <div class="widget-content">
            <table class="table table-striped table-bordered table-hover">
              <thead>
                <tr>
                  ...
                  <!--<th>Category Icon</th>-->
                  <th>Date</th>
                 <!-- <th>Show On Registration Page</th>-->
                  <th>Actions</th>
                </tr>
              </thead>
              <tbody>
			...
			if(isset($_REQUEST['delete']) && $_REQUEST['delete']==1)
			{
				...
				$obj_query->query_execute("delete from cms_subcategory_backoffice where id='$c_id'");
			}
			...
			if(isset($_REQUEST['search']))
			{
				...
				$url='admin_main.php?page_number=168&'.$query_string;
			}
			else
			{
				$url='admin_main.php?page_number=168&'.$search_string;
			}
			...
			if($per_page != "all"){
			  ...
			  $res_products_tol=$obj_query->query("id","cms_subcategory_backoffice","1=1 $search_string $con_search order by category_name ");
			  $total_row=$obj_query->num_row($res_products_tol);
			  $pages = ceil($total_row/$per_page);
						 
			  $res_cat=$obj_query->query("*","cms_subcategory_backoffice"," 1=1 $search_string $con_search order by category_name $limit");
			...
			while($row_cat=$obj_query->get_all_row($res_cat))
			{
			  ?>
                <tr>
                  <td><?php echo $s_no; ?></td>
                  <td><?php echo $obj_query->get_field_name("cms_category_backoffice","category_name","id='$row_cat[category_id]'");?></td>
                  <td><?php echo ucwords($row_cat['category_name']); ?></td>
                  <td><?php echo $row_cat['link']; ?></td>
                  <td><?php echo $row_cat['target']; ?></td>
                  <td><?php if($row_cat['link_status']) echo "Inactive"; else echo "Active"; ?></td>
                  <!--<td><img src="<?php echo SITE_URL."images/".$row_cat['icon']; ?>" /></td>-->
                  <td><?php echo $row_cat['add_date']; ?></td>
                 <!-- <td><input type="checkbox" name="showonreg[]" <?php if($row_cat['show_reg_page']){ echo "checked";}?> value="<?php echo $row_cat['id'];?>" /></td>-->
                  <td>&nbsp;
                  <span><a href="admin_main.php?page_number=169&category_id=<?php echo $row_cat['id'];?>&category_name=<?php echo $row_cat['category_name'];?>" title="Edit"><img src="../images/edit.png" /></a></span>
                  <span style="margin-left:5px;"><a href="admin_main.php?page_number=168&delete=1&category_id=<?php echo $row_cat['id'];?>" onclick="if(confirm('Do You Want To Delete')){ return true;}else{ return false;}" title="Delete"><img src="../images/Trashcan.png" /></a></span>
                  </td>
                </tr>
                <?php 
					$s_no++;
...
alert(catids);*/
	var formData="action=showonreg&catids="+catids;
	//alert(formData);
    $.ajax({
        url: 'ajax.php',
        data: formData,
