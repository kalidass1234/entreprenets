...
function registernow($payment_mode)
{
	...
	/*************  Commission For Level-Income Table For Fast Start Bonus AND update RANK-BV Table FOR POINT VALUE  *****************/
		...
		if($user_id!='')
		{
			...
			{
				$makeBoard = $Commission->makeBoard($user_id,$ref,$nom_id_70,$packageAmnt);
			}
			else if($nom_id_140!='')
			{
				$makeBoard = $Commission->makeBoard($user_id,$ref,$nom_id_140,$packageAmnt);	
			}
