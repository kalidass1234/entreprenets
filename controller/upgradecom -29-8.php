...
function upgrade_com($invoice, $closing) {
	...
	$result_reftype=mysql_fetch_array(mysql_query("SELECT mem.user_type, reg.astatus from membership mem INNER JOIN registration reg ON reg.user_id=mem.user_id where mem.user_id='$ref_id'  and DATE(date)+INTERVAL 1 YEAR>='$date'"));
			  ...
			  $temp_dynamic=$res_dynamic_aff['nom_id'];
				while($temp_dynamic!='cmp'){
					...
					if($date_refl>$date && $query_levelc['astatus']==1){
						...
						$mstatus=$result_ref['mstatus'];
						if($memstatus==0 && $mstatus==0){
							mysql_query("INSERT INTO level_income(income_id , down_id, package, commission, remark,l_date,status, com_type, invoice, percent, price, level, payout_date, closing_no) values ('$temp_dynamic', '$id' ,'Valunaire Subscription', '$levelc', 'Dynamic Level Based Residual Commission',CURDATE(),'1', 'r', '$invoice', '$percentl', '$plan_amt11m', '$il', CURDATE(), '$closing')");
  ...
  $temp_var_fast=2;
   ...
   $temp_fast=$result_fast['ref_id'];
	  ...
	  { $percent=7.5;
		...
		$inslev56="insert into  level_income (income_id , down_id, package, commission, remark,l_date,status, com_type, level, invoice, price, percent, payout_date, closing_no) values ('$ref_id', '$id' ,'Product Bundle Purchase', '$onemonthcomm', 'Direct Residual Bonus',CURDATE(),'1', 'a', 1, '$invoice', '$plan_amt', '$percent', CURDATE(), '$closing')";
	}
	  $memstatus= $result_ref['mem_status'];
	$mstatus=$result_ref['mstatus'];
	 if($memstatus==0 && $mstatus==0){
...
// '<br>';
	...
	if($flag==2){ $flag_fast=1;
			/****** FAST START BONUS  *******/
			 ...
			 $ref_id_fast=$ref_id;
			while($idnom!='cmp'){
			 "<br>SELECT subs_date, ref_id, nom_id, reg.user_id, mem_status, mstatus, fewa.amount FROM registration reg INNER JOIN final_e_wallet fewa on fewa.user_id=reg.user_id  WHERE reg.user_id='$ref_id_fast'";
				$query_fast=mysql_fetch_array(mysql_query("SELECT subs_date, ref_id, nom_id, reg.user_id, mem_status, mstatus, fewa.amount FROM registration reg INNER JOIN final_e_wallet fewa on fewa.user_id=reg.user_id  WHERE reg.user_id='$ref_id_fast'"));
				$date_ref=$query_fast['subs_date'];
				$ref_id_fast=$query_fast['ref_id'];
				$user_id_fast=$query_fast['user_id'];
				...
				//	 "INSERT INTO level_income SET income_id='$user_id_fast' , down_id='$id', package='Product Purchase', commission='$fsb', remark='Fast Start Bonus',l_date=CURDATE(),status='1', com_type='r', invoice='$invoice', percent='$percent', level='$i', price='$plan_amt', payout_date=CURDATE(), closing_no='$closing' ";
					 if($memstatus==0 && $mstatus==0){  "<br>INSERT INTO level_income SET income_id='$user_id_fast' , down_id='$id', package='Product Purchase', commission='$fsb', remark='Fast Start Bonus',l_date=CURDATE(),status='1', com_type='r', invoice='$invoice', percent='$percent', level='$i', price='$plan_amt', payout_date=CURDATE(), closing_no='$closing' ";
							...
							$fbalance1=$query_fast[amount]+$fsb;
									/*$fbalance1=round($fbalance1,2);*/
								 "<br>insert into credit_debit (user_id, credit_amt,  sender_id, receive_date,Remark,status,ttype,TranDescription,final_bal) values('$user_id_fast','$fsb','Valunaire Admin',CURDATE(),'Fast Start Bonus Earned','0','$user_id_fast','Fast Start Bonus Earned','$fbalance1')";
							...
							if($date_refm>$date && $query_levelm['gstatus']==1){
								...
								$mstatus=$query_levelm['mstatus'];
								 if($memstatus==0 && $mstatus==0){
									...
									$fbalance1=$query_levelm[amount]+$matchc;
									/*$fbalance1=round($fbalance1,2);*/
									 "<br>insert into credit_debit (user_id, credit_amt,  sender_id, receive_date,Remark,status,ttype,TranDescription,final_bal) values('$userid_match','$matchc','Valunaire Admin',CURDATE(),'Matching Bonus','0','$userid_match','Matching Bonus','$fbalance1')";
