...
require_once "commissionFunc.php";
	...
	function makeBoardBMC3($user_id,$ref,$nom_bmc3,$package_amnt)
	{
		...
		while($nom_bmc3!='cmp')
		{
			...
			$fetchMaxBoardRR = boardMainTableOfUser($nom_bmc3,$package_amnt);
			...
			$maxCellNumber = maxCellNumberA($nom_bmc3,$package_amnt);
			$maxCellRR = $maxCellNumber['max_cell'];
			$newCellNumber = $maxCellRR + 1;

			if($newCellNumber>13)
			{
				$fetchMaxRR = fetchBoard($nom_bmc3,$package_amnt);
			...
			else
			{
				$fetchMaxRR = fetchBoard($nom_bmc3,$package_amnt);
				$newBoardNumber = $fetchMaxRR['board_no'];
				$maxCellNumber = maxCellNumber($nom_bmc3,$package_amnt);
			...
			if($nom_bmc3!='cmp' && $nom_bmc3!=0  && $ref!='cmp')
			{
				$qryInsertBS = "INSERT INTO board_status SET board_number='".$newBoardNumber."', package_id ='$package_amnt', cell_no='".$newCellNumber."', board_owner_id='".$ref."', user_id='".$user_id."',upper_id='".$nom_bmc3."',level ='$i'"; 
				$exeQryInsertBS = mysql_query($qryInsertBS);
			}
			else
			{
			...
			if($newCellNumber == 13)
			{
				$fetchResult = maxBoardNumber();
				$boardNumbers = $fetchResult+1;
				$updateBoardMain = "UPDATE board_main SET board_status='1',board_date='$current_date' WHERE user_id='".$nom_bmc3."' AND package_id ='$package_amnt'";
				$exeQryUpdateBM = mysql_query($updateBoardMain);
				
				$updateBoardMain = "UPDATE board_status SET cell_status_bmc3 = '1' WHERE 1 AND (upper_id='".$nom_bmc3."' OR upper_id='0') AND package_id ='$package_amnt'";
				$exeQryUpdateBM = mysql_query($updateBoardMain);

				$qryInsertBM = "INSERT INTO board_main SET board_status='0', board_no='$boardNumbers', board_date='0000-00-00', user_id='".$nom_bmc3."', package_id ='$package_amnt'";
				$exeQryInsertBM = mysql_query($qryInsertBM);

				$updateMainRegis = "UPDATE registration SET cell_status_bmc3 = '1' WHERE 1 AND  ref_id ='$ref'";
				$exeQryUpdateBM = mysql_query($updateMainRegis);
			}

			$selectNom = mysql_query("select nom_id,nom_bmc3,ref_id from registration where user_id='$nom_bmc3'  ");
			$fetchNom = mysql_fetch_array($selectNom);

			if($fetchNom['nom_id']!='0' && $fetchNom['nom_id']!='' && $ref!='cmp')
			{
				$nom_bmc3=$fetchNom['nom_id'];
			}
			if($fetchNom['nom_bmc3']!='0' && $fetchNom['nom_bmc3']!='' && $ref!='cmp')
			{
				$nom_bmc3=$fetchNom['nom_bmc3'];
			}
			else
			{
				break;
	...
	function countNomTotalBMC3($user_id)
	{
		$nomArray = array();
		$selectNomIds = "select nom_bmc3,nom_id from registration where (user_name='$user_id' OR user_id='$user_id')";
		...
		while($nom_bmc2 !="cmp")
		{
			$selectNomId = "select nom_bmc3,nom_id from registration where user_id='$nom_bmc2'";
			$resNomId=mysql_query($selectNomId);
			$rowNomId=mysql_fetch_assoc($resNomId);

			if(($rowNomId['nom_bmc3'] !='cmp' && $rowNomId['nom_bmc3'] !=0) || ($rowNomId['nom_id'] !='cmp' && $rowNomId['nom_id'] !=0))
			{
				if($rowNomId['nom_id']!='')
				{
	...
	function matrixBonusBMC3($user_id)
	{
		$bonus = "1";
		...
		/*------ Fetch PACKAGE Info of Registerd User END ------*/
                $usdAmount = 0;
		$countNomTotal = countNomTotalBMC3($user_id);
		$i=1;
		foreach($countNomTotal as $records)
		{
			...
			if($i <3)
			{
				if($i==1)
				{
					...
					if(($packageAmountNom == 35.00 && $userPrice == 140.00))
					{
						$first_level_comm = 30;
					}
					else if(($packageAmountNom == 70.00 && $userPrice == 35.00))
					{
						$first_level_comm = 30;
					}
					else if(($packageAmountNom == 140.00 && $userPrice == 70.00))
					{
						$first_level_comm = 15;
					}
