...
class User_Login
{
	function login($uname,$pass,$return_page,$current_page,$type='')
	{
			...
			if($res>0 && $uname!='' && $pass!='')
			{
				...
				if(isset($_SESSION['adid']))
				{
					...
					$update="update registration set userIP='$login_ip', last_login=now() where user_name='$uname'";
	...
	{
		$qry=mysql_query("select first_name,last_name,mobile,phoner,address1,address2,city,state,zip,country from registration where user_name='$user_name'") or die(mysql_error());
	...
	function getUserShippingAddress($user_name)
	{
		$qry=mysql_query("select * from shipping_address where user_name='$user_name'");
		$num=mysql_num_rows($qry);
		if($num>0)
		{
		$row=mysql_fetch_assoc($qry);
	...
	function getShippingAddressType($user_name)
	{
		
	
		$qry=mysql_query("select shipping_address_type from purchase_detail where user_name='$user_name'");
		$num=mysql_num_rows($qry);
		if($num>0)
		{
		$row=mysql_fetch_assoc($qry);
		return $row['shipping_address_type'];
		}
		else
		{
			return 0;
...
class mysql_func
{
	...
	{
		echo $sql="select ".$select." from ".$table1." inner join ".$table2." on ".$on." where ".$where." order by ".$order_by;
	...
	function _get_member_rank($user_name)
{
	...
	function mem_pos($ref_id123,$posi)
	{
		if($posi=='automatic')
		{
			    ...
			    $sql_left_count="select * from level_income where income_id='$ref_id123' and position='left'";
				$res_left_count=mysql_query($sql_left_count);
				$count_left_count=mysql_num_rows($res_left_count);
				// find the right leg count
				$sql_right_count="select * from level_income where income_id='$ref_id123' and position='right'";
	...
	function _user_registration()
	{
		...
		// check sponsor
                if($_POST['captcha']!=($_POST['num1']+$_POST['num2']))
                { header("Location:registeration.php?msg=Wrong Captcha."); exit; }
		$table_name=TABLE_PREFIX.'registration';
		...
		else
		{
			...
			$res_p=$this->mysql_func->query("placement_id,placement_id_status,power_leg","registration","user_id='$ref_id123'");
		...
		if($_POST['nom_id']!='' && $_POST['position']!='')
					{
					$query1="select * from registration where nom_id='".$_POST['nom_id']."' and binary_pos='$posi'";
		$result1=mysql_query($query1);
		$row=mysql_fetch_array($result1);	
		$uui=$row['user_id'];
		if($uui=='')
		...
		{
					$posi=$row_p['power_leg'];
					if($posi=='automatic')
					{
					 $posi=$this->mem_pos($ref_id,$posi);	
					}
		...
		{
			header("Location:registration.php?msg=Wrong Username or Wrong Sponser Detail");
		}
	}
	function _registration_mail($user_id)
	{
		...
		$res=mysql_query($sql);
		$row=mysql_fetch_assoc($res);
		$from="divya@maxtratechnologies.com"; // shopdeal admin username
		$email=$row['email'];
...
<div style='width:800px; margin:0px auto;'>
  ...
  <tr>
    <td><table width='100%' border='0' cellspacing='0' cellpadding='0'>
      <tr>
        ...
        <td width='43%' align='left'><img src='http://198.154.192.169/~divya/grenature_new/img/logo-1.jpg'/></td>
		<td width='57%'>&nbsp;</td>
      </tr>
    </table></td>
  </tr>
  <tr>
    <td><table width='100%' border='0' cellspacing='0'cellpadding='0'>
      <tr>
        <td width='3%'>&nbsp;</td>
        <td width='93%'><P style='font-family:Calibri; font-size:16pt; font-weight:bold; font-style:italic; color:#00;'>Welcome ".stripslashes(htmlentities($row['user_name'])).",</P></td>
        <td width='4%'>&nbsp;</td>
      </tr>
      <tr>
        <td height='50'>&nbsp;</td>
        <td height='50'><p style='font-family:Calibri; font-size:16pt; font-weight:bold; font-style:italic; color:#3399FF; padding: 5px 0px '>Congratulations! You successfully completed your Registration</p></td>
        <td height='50'>&nbsp;</td>
      </tr>
      ...
      <tr>
        ...
        <td height='100'><table width='100%' border='0' cellspacing='0' cellpadding='0'>
          <tr>
            <td width='24%'><p style='font-family:Calibri; font-size:14pt; color:#000; font-weight:bold;'>Username: </p></td>
            <td width='76%'><p style='font-family:Calibri; font-size:14pt; font-weight:normal; color:#000;'>".stripslashes(htmlentities($row['user_name']))."</p></td>
          </tr>
          <tr>
            <td><p style='font-family:Calibri; font-size:14pt; color:#000; font-weight:bold;'>User ID</p></td>
            <td><p style='font-family:Calibri; font-size:14pt; font-weight:normal; color:#000;'>".stripslashes(htmlentities($row['user_id']))."</p></td>
          </tr>
          <tr>
           <td><p style='font-family:Calibri; font-size:14pt; color:#000; font-weight:bold;'>Password: </p></td>
            <td><p style='font-family:Calibri; font-size:14pt; font-weight:normal; color:#000;'>".stripslashes(htmlentities($row['user_pass']))."</p></td>
          </tr>
          <tr>
            <td><p style='font-family:Calibri; font-size:14pt; color:#000; font-weight:bold;'>Transaction Pin: </p></td>
            <td><p style='font-family:Calibri; font-size:14pt; font-weight:normal; color:#000;'>".stripslashes(htmlentities($row['t_code']))." </p></td>
          </tr>
		  
		  <tr>
            <td><p style='font-family:Calibri; font-size:14pt; color:#000; font-weight:bold;'>Refferal Link: </p></td>
            <td><p style='font-family:Calibri; font-size:14pt; font-weight:normal; color:#000;'><a href='$url'>".stripslashes(htmlentities($url))."</a></p></td>
          </tr>
      ...
      <tr>
        <td>&nbsp;</td>
        <td><p style='font-family:Calibri; font-size:16pt; font-weight:bold; font-style:italic; color:#000; padding:5px 0px;'>Cheers to your Success!</p></td>
        <td>&nbsp;</td>
      </tr>
      
      <tr>
        <td>&nbsp;</td>
        <td><p style='font-family:Calibri; font-size:14pt; color:#000; font-weight:bold; font-style:italic;'>Grenature Admin</p></td>
        <td>&nbsp;</td>
      </tr>
    </table></td>
  </tr>
 ...
 //echo $msg;exit;
	  mail($email,'User Registration',$msg,$headeruser1,'$from');
	  return $user_id;
	}
