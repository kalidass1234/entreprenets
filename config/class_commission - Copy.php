...
class Class_Commission extends mysql_func
{
	...
	function Direct_Referral_Bonus($username,$user_id,$ref_id,$total,$total_bv,$invoice_no)
	{
		...
		$date=date('Y-m-d');
			...
			"date"=>$date,
			"payout_date"=>$date,
			"remark"=>'Get '.$comRemark.' % Commission On The Invoice'.$invoice_no,
			...
			// update cash wallet
			$this->query_execute("update final_e_wallet set amount=amount+$commission where user_id='$ref_id'");
			...
			"sender_id"=>'admin',
			"receive_date"=>$date,
			"TranDescription"=>'Get  '.$comRemark.' % Commission On The Invoice'.$invoice_no,
			"Remark"=>'Get  '.$comRemark.' % Commission On The Invoice'.$invoice_no,
			"invoice_no"=>$invoice_no
			);
...
function Upgrade_Bonus($username,$user_id,$ref_id,$total,$total_bv,$invoice_no)
	{
		$q=mysql_query("select user_name from registration where user_id='$ref_id'") or die(mysql_error());
		$r=mysql_fetch_assoc($q);
		$uname=$r['user_name'];
		...
		$date=date('Y-m-d');
			...
			"date"=>$date,
			"payout_date"=>$date,
			"remark"=>'Get '.$comRemark.' Cashback On The Invoice'.$invoice_no,
			"com_percent"=>15,
			"invoice_bv"=>$total_bv,
			"commission_bv"=>$commission_bv
			);
			...
			"sender_id"=>'admin',
			"receive_date"=>$date,
			"TranDescription"=>'Get  '.$comRemark.' Cashback On The Invoice'.$invoice_no,
			"Remark"=>'Get  '.$comRemark.' Cashback On The Invoice'.$invoice_no,
			"invoice_no"=>$invoice_no
			);
			$this->insert_tbl($inserarr,"credit_debit");	
...
{
$sql="select sum(invoice_bv) as total from purchase_history where (purcheser_id='$user_id' and income_id='$user_id') ";
	$res=mysql_query($sql) ;
	...
	function _update_product_volume($id,$invoice_bv,$invoice_no)
	{
		...
		if($invoice_bv)
		{
			...
			else if($leg==3)
			{
			   ...
			   $sql_left_count="select sum(invoice_bv) as left_volume from purchase_history where income_id='$id' and leg='left'";
				...
				$sql_right_count="select sum(invoice_bv) as right_volume from purchase_history where income_id='$id' and leg='right'";
