			...
			<div class="grid_12">
				<div class="widget_wrap">
					...
					?>
						...
						if($_POST['Show']=='Search')
						{
						$query="SELECT *, sum(price*quantity) as oto FROM purchase_detail PUR inner join product_category PRO where PRO.p_cat_id=PUR.p_id  and user_id='$id' and PUR.section='eshop' and PUR.false_invoice=1 and PUR.date BETWEEN '$dt1' and '$dt2' group by invoice_no order by PUR.date desc  ";
						}
						else
						$query=" SELECT *, sum(price*quantity) as oto FROM purchase_detail PUR inner join product_category PRO where PRO.p_cat_id=PUR.p_id  and user_id='$id'  and PUR.section='eshop' and PUR.false_invoice=1 group by invoice_no order by PUR.date desc    ";
		//echo $query;				
