...
<div id="container">
	...
	<div id="content">
        ...
        <th >&nbsp;</th>
        <th >Invoice No.</th>
        <th><input type="text" name="invoice_no" value="<?php echo $_POST['invoice_no'];?>"></th>
        <th >&nbsp;</th>
        <th ><button name="show" type="submit" class="btn_small btn_gray" value="Search" onClick="shoLst(txtfrm.value, txttil.value);" onMouseOver="formCheck();">Search</button></th>
						...
						{
						$con_search="";
						if($_REQUEST['txtfrm']!='' && $_REQUEST['txtfrm']!='')
						{
							$con_search.=" and (PUR.date BETWEEN '$dt1' and '$dt2') ";
						}
						if($_REQUEST['invoice_no']!='')
						{
						$invoice_no=$_REQUEST['invoice_no'];
							$con_search.=" and (PUR.invoice_no='$invoice_no')";
						}
						$query="SELECT *, sum(price*quantity) as oto,PUR.status FROM purchase_detail PUR inner join product_category PRO where  PRO.p_cat_id=PUR.p_id  and user_id='$id' $con_search group by invoice_no order by PUR.pd_id desc  ";
						}
						else
						$query=" SELECT *, sum(price*quantity) as oto,PUR.status FROM purchase_detail PUR inner join product_category PRO where  PRO.p_cat_id=PUR.p_id  and user_id='$id' $con_search group by invoice_no order by PUR.pd_id desc    ";
//echo $query;				
...
//echo $query;
					...
					?>
						<tr>
							...
							<td class="sdate center">
								 <?php echo date("m-d-Y", strtotime($noticia['date']));?>
							</td>
							<td class="center">
								<?php echo CURRENCY.' '.round($noticia['oto'],2);?>
							</td>
							
							<td class="sdate center">
								<?=CURRENCY.' '.round($net_amount,2);?>
							</td>
