...
/*	 var val = [];
    ...
    if(ss!='')
	{
	  ...
	  alert(data+' message delete from trash');
			...
			<div class="grid_12 full_block">
				<div class="widget_wrap tabby">
					<div class="widget_top">
						...
						<div id="widget_tab">
							<ul>
								...
								<li><a href="#tab3"  class="active_tab">Trash(<?php echo $count_trash;?>)</a></li>
							</ul>
						</div>
					</div>
					...
					<div class="widget_content">
						...
						<h6 style="margin-top:0px; margin-left:-22px;"><!--<span><a class="action-icons c-delete" href="javascript:deletemessage();" title="Delete"></a><span onClick="javascript:deletemessage();" style="color:#0099CC; font-size:14px; cursor:pointer; margin-left:10px;">Delete Message</span>-->
						...
						<?php
						  ...
						  $i=1;
						  while($x1=mysql_fetch_array($res_trash))
						  {
						  
			 			 ?>
						<tr>
							<td class="center tr_select ">
								<input name="checkbox[]" type="checkbox" value="<?=$x1[id].'_'.$x1[type];?> ">
							</td>
							...
							<td class="center tr_select ">
								<a  href="view-message.php?type=<?php echo $x1['type'];?>&id=<?=$x1['id']?>" title="View"><?=$x1['reciever_name']?> </a>
							</td>
							<td class=" center" >
								 <a  href="view-message.php?type=<?php echo $x1['type'];?>&id=<?=$x1['id']?>" title="View"><?=$x1['subject']?></a>
							</td>
							<td class="center" style="border-right:none;"><a  href="view-message.php?type=<?php echo $x1['type'];?>&id=<?=$x1['id']?>" title="View"><?php echo substr($x1['message'],0,20);?></a></td>
							<td class="center">&nbsp;</td>
							<td class="center sdate">
								 <span class=" "><a  href="view-message.php?type=<?php echo $x1['type'];?>&id=<?=$x1['id']?>" title="View"><?php
								 $curdate=date('Y-m-d');
								 $sentdate=date('Y-m-d', strtotime($x1['msg_date']));
								 $curtime=strtotime($curdate);
								 $senttime=strtotime($sentdate);
								 if($senttime==$curtime){  echo date('H:i', strtotime($x1['ts']));}
								 else { echo date('d M, Y',strtotime($x1['msg_date']));}
							...
							<td class="center">
								<span><a class="action-icons c-approve" href="view-message.php?type=<?php echo $x1['type'];?>&id=<?=$x1['id']?>" title="View">View</a></span>
							</td>
						</tr>
						<?php $i++; } ?>
