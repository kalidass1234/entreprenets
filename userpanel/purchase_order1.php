...
$dt1="$yy1-$mm1-$dd1";
$dt2="$yy2-$mm2-$dd2";
$query2="SELECT * FROM purchase_detail PUR inner join product_category PRO on PRO.p_cat_id=PUR.p_id where user_id='$id' and PUR.status in (1,2) and PUR.section='market_place' and (PUR.date BETWEEN '$dt1' and '$dt2' ) group by invoice_no order by PUR.date desc";
}
else
$query2="SELECT * FROM purchase_detail PUR inner join product_category PRO where PRO.p_cat_id=PUR.p_id and PUR.status in (1,2) and PUR.section='market_place' and user_id='$id' group by invoice_no order by PUR.date desc";
//echo $query2;
...
<div id="container">
	...
	<div class="page_title">
		...
		<h3>Market Place Purchase History</h3>
			...
			<div class="grid_12">
				<div class="widget_wrap">
					<div class="widget_top">
						...
						<h6>Market Place Purchase History</h6>
					</div>
					<div class="widget_content">
						...
						if($_POST['Show']=='Search')
						{
						$query="SELECT *, sum(price*quantity) as oto FROM purchase_detail PUR inner join product_category PRO where PRO.p_cat_id=PUR.p_id and PUR.status in (1,2) and PUR.section='market_place' and user_id='$id' and PUR.date BETWEEN '$dt1' and '$dt2' group by invoice_no order by PUR.date desc  ";
						}
						else
						$query=" SELECT *, sum(price*quantity) as oto FROM purchase_detail PUR inner join product_category PRO where PRO.p_cat_id=PUR.p_id and PUR.status in (1,2) and PUR.section='market_place' and user_id='$id' group by invoice_no order by PUR.date desc    ";
		//echo $query;				
					...
					?>
						<tr>
							...
							<td class="sdate center">
								<a href="invoice.php?invoice_no=<?=$noticia[invoice_no]; ?>"><?=$noticia[invoice_no];?></a>
							</td>
							<td class="sdate center">
							...
							<td class="center">
								<a href="invoice.php?invoice_no=<?=$noticia[invoice_no]; ?>"><span class="badge_style b_done">View Detail</span></a>
							</td>
							<td class="center">
								<a href="feedback.php?invoice_no=<?=$noticia[invoice_no]; ?>"><span class="badge_style b_done">Leave Feedback</span></a>
							</td>
							<td class="center">
								<?php
								if($noticia['false_invoice']==0)
								{
									...
									{
									?>
									<a href="cancel_invoice.php?invoice_no=<?=$noticia[invoice_no]; ?>&target=request" onClick="if(confirm('Do You Want To Cancel Invoice.')){ return true;}else{ return false;}"><span class="badge_style b_done">Cancel</span></a>
									<?php
									}
									else
									{
										...
										if($status==0)
										{
											echo "Cancel Invoice Request Sent";
										}
										else if($status==1)
										{
							...
							<td class="center">
								<!--<span class="badge_style b_high">--><div class="btn_30_light">
										<a href="invo_detail_down.php?invoice_no=<?=$noticia[invoice_no]; ?>" title="Download .doc"><span class="icon drive_disk_co"></span></a>
									</div><!--</span>-->
							</td>
						</tr>
