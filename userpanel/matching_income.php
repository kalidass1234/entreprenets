		  ...
		  <div class="grid_12">
			  ...
			  <div class="widget_wrap">
					<div class="widget_top">
						...
						<h6>Matching Bonus Details
                        
                         <span style="margin-left:600px; "> <a href="exportMatching.php">Export</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:void();" onclick="coderHakan()">Print</a>
<!--                        <img src="../images/excel.png" onClick="document.forms['form1'].submit();" name="getReport" id="getReport" >
                       ...
                       <img src="../images/print.png"  onClick="printDiv('example');">-->
						...
						$heading.="<td>Date</td><td>Left Carry Forword</td><td>Right Carry Forword</td><td>Left Match</td><td>Right Match</td><td>Cycle</td><td>Bonus(BV)</td><td>Bonus($)</td><td>TDS(%)</td><td>TDS Amount</td><td>Miscellaneous(%)</td><td>Miscellaneous Amount</td><td>Final Bonus(USD)</td>
</tr>";
						
						$heading3="Date,Left Carry Forword,Right Carry Forword,Left Match,Right Match,Cycle,Bonus(BV),Bonus($),TDS(%),TDS Amount,Miscellaneous(%),Miscellaneous Amount,Final Bonus(USD)";
						
						$record = "b_date,carry_fwd_left,carry_fwd_right,match_left,match_right,income_pair,commission_bv,commission,tds_percent,tds_amount,miscellaneous_percent,miscellaneous_amount,final_amount";
						$table="binary_income";
                      ...
                      <form id="form1" name="form1" method="post" action="ExportExcel.php">
                       ...
                       <label></label></form>
                        ...
                        <form id="form2" name="form2" method="post" action="ExportDoc.php">
                       <input type="hidden" name="record" value="<?php echo $record; ?>">
                        <input type="hidden" name="con" value="<?php echo $con; ?>">
                       <input type="hidden" name="table" value="<?php echo $table; ?>">
...
<script>function coderHakan(){var sayfa = window.open('','','width=800,height=800');sayfa.document.open("text/html");sayfa.document.write(document.getElementById('printArea').innerHTML);sayfa.document.close();sayfa.print();
					...
					<div class="widget_content" id="example">
						<!--<h3>Content Table</h3>
						<p>
						...
						//$sql="select * from binary_income where income_id='$id' and level<>0 order by l_id desc";
						$sql="SELECT * FROM binary_income WHERE income_id = '$id' AND bonus_name='2' ORDER BY id DESC";
 //echo $sql;
						$res=mysql_query($sql);
						?>
                        ...
                        <tr>
                          ...
                          <th>TDS Amount(10%)</th>
                          <th>Trust Amount(5%)</th>
                          <th>Admin Amount(5%)</th>
                          <th>Bonus (Rs)</th>
						...
						{
							$select = "select user_name from registration where (user_id='".$row['matching_id']."') ";
							$resNomId=mysql_query($select);
							$rowNomIdCount=mysql_fetch_array($resNomId);
							$rowNomIdCount['user_name'];
							$totc=$totc+$row['final_amount'];
						?>
						<tr>
						<td class="center">	<?php echo $ii;?> </td>
						<td class="center">	<?php echo date('m/d/Y',strtotime($row['b_date']));?></td>
                        <td class="center"> <?php echo $row['matching_id'];?> </td>
                        <td class="center"> <?php echo $rowNomIdCount['user_name'];?> </td>
						<td class="center"><?php echo $row['tds_amount'];?></td>
						<td class="center"><?php echo $row['trust_amount'];?></td>
						<td class="center"><?php echo $row['admin_amount'];?></td>
                        <td class="center"><?php echo $row['commission'];?></td>
						<td class="center"><?php echo $row['final_amount'];?></td>
                        <td class="center"><?php if($row['status']==2) echo "Lost"; else echo "Paid";?></td>
						</tr>
						<?php
						$ii++;
						}
