...
/*	 var val = [];
    ...
    if(ss!='')
	{
		var urldata="ss="+ss+"&target=inbox";
		$.ajax({
		type :'POST',
					...
					<div class="widget_top">
						...
						<div id="widget_tab">
							<ul>
								...
								<li><a href="#tab1" class="active_tab">Inbox(<?php echo $count11;?>)</a></li>
					...
					?>
					 ...
					 <form id="form_send_box1" name="form_send_box" action="delete_message_func1.php" method="post">
						...
						<tr>
							...
							<th >Message</th>
							<th style="border-right:none;">Attach File</th>
							<th>&nbsp;</th>
							<th>View</th>
						...
						<?php
			 			 ...
			 			 ?>
							<tr>
                                  <td align="center" class="ptext tr_select"><input type="checkbox" name="checkbox[]" id="checkbox[]" value="<?=$x11['id']?>"/></td>
                                  <td align="center" class="ptext"><a href='view-message.php?inbox=inbox&id=<?=$x11['id']?>'><?php
								 $curdate=date('Y-m-d');
								 $sentdate=date('Y-m-d', strtotime($x11['msg_date']));
								 $curtime=strtotime($curdate);
								 $senttime=strtotime($sentdate);
								 if($senttime==$curtime){ echo date('H:i:s', strtotime($x11['ts']));}
								 else {  echo date('d M, Y',strtotime($x11['ts']));}
								//echo date('d M, Y',strtotime($x11['ts']));?></a></td>
                                  <td align="center" class="ptext"><a href='view-message.php?inbox=inbox&id=<?=$x11['id']?>'><?=$x11['sender_name']?></a></td>
                                  <td align="center" class="ptext"><a href='view-message.php?inbox=inbox&id=<?=$x11['id']?>'><?=$x11['subject']?></a></td>
                                  <td align="center" class="ptext" ><a href='view-message.php?inbox=inbox&id=<?=$x11['id']?>'>
                                        <?php echo substr($x11['message'],0,20);?>
                                      </a></td>
                                 <td align="center" class="ptext" style="border-right:none;"><?php if($x11['file_name']){?><a href="attachfile/<?=$x11['file_name']?>" target="_blank">Attach File</a><?php } else{ echo "No Attachment.";}?></td>
								 <td>&nbsp;</td>
								 <td class="center">
								<span><a class="action-icons c-approve" href="view-message.php?inbox=inbox&id=<?=$x11['id']?>" title="View">View</a></span>
							</td>
