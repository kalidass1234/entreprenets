...
<?php
if(showuserid($_SESSION['SD_User_Name'])==1234567)
{
...
$sql="SELECT invoice_amt, commission, purcheser_id, remark, level, invoice_amt,STATUS , paid_status, ts,date,user_level FROM `level_income_admin` WHERE income_id = 'guru' AND level <>0 and status=0 and paid_status=0 ORDER BY `ts` DESC";
$res=mysql_query($sql);
?>
                ...
                <td class="center"><?php
				$t=date('m');
				$d=date('m',strtotime($row['date']));
				 if($row['status']==2 && $row['paid_status']==2){ echo "Paid";} else if($row['paid_status']==4){ echo "Cancel";} else if($row['status']==0 && $row['paid_status']==0){ echo "Unpaid";} /*else if($row['status']==0 && ($t==$d)){ echo "Pending";} else if($row['status']==0 && ($t>$d)){  echo "Unpaid";}*/?></td>
						</tr>
						<?php
						$ii++;
						}
 ...
 {
 	echo "You  are not authorize to access this section.";
 }
 ?>                       
					</div>
				</div>
