			...
			<div class="grid_12">
				<div class="widget_wrap">
					...
					?>
						...
						if($_POST['Show']=='Search')
						{
						$query="SELECT *, sum(price*quantity) as oto FROM purchase_detail PUR inner join product_category PRO where PUR.section='marketing_product' and PRO.p_cat_id=PUR.p_id  and user_id='$id' and PUR.date BETWEEN '$dt1' and '$dt2' group by invoice_no order by PUR.date desc  ";
						}
						else
						$query=" SELECT *, sum(price*quantity) as oto FROM purchase_detail PUR inner join product_category PRO where PUR.section='marketing_product' and PRO.p_cat_id=PUR.p_id  and user_id='$id' group by invoice_no order by PUR.date desc    ";
		//echo $query;				
