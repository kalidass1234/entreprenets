...
<div id="container">
	...
	$sql_ref_subs="select * from subscription where user_id='$id' and status=0 and type='3'";
	$res_ref_subs=mysql_query($sql_ref_subs);
	$count_ref_subs=mysql_num_rows($res_ref_subs);
	$row_ref_subs=mysql_fetch_assoc($res_ref_subs);
	
	$reg_date_30=date("m/d/Y",strtotime(date("Y-m-d", strtotime($row_ref_subs['subs_date'])) . " +1 year"));
	?>
	<div id="content">
		<div class="grid_container"><span class="clear"></span>
          ...
          <strong>UserId:</strong><?php echo $id;?>
          </p>
          <?php
          }
		  ?>
		  ...
		  {
		  ?>
          <h6 align="center" style="color:#CC0000; font-family:Arial, Helvetica, sans-serif; font-size:12px" id="showval">
...
function GetCount()
{
	...
	var goLive = new Date('<?php echo $reg_date_30;?>'); // target date
	...
	var diffSecs = roundDown((((diffMs % 86400000) % 3600000) % 60000) / 1000 ); // se 
	//alert(diffDays);
	//$("#countbox").html(diffDays);
	//document.getElementById('showval').innerHTML=diffDays+" Days Left";
	//$("#showval").html("message");
	var out="";
	if(diffDays != 0){out += diffDays +" day"+((diffDays!=1)?"s":"")+", ";}
                if(diffDays != 0 || diffHrs != 0){out += diffHrs +" hour"+((diffHrs!=1)?"s":"")+", ";}
                if(diffDays != 0 || diffHrs != 0 || diffMins != 0){out += diffMins +" minute"+((diffMins!=1)?"s":"")+", ";}
                out += diffSecs +" seconds";
                document.getElementById('showval').innerHTML=out;
...
?>
			  <div class="widget_wrap">
					...
					<div class="widget_content">
						...
						<?php
						$arr_package=array('','Discount Benifit Members','Discount Benifit Members+Residual Income','Affiliated Refferal');
						if($category_three)
						{
							$sql_ref_subs1=mysql_query("select * from subscription where user_id='$id' and status=0 and type in ('2','3')");
							$count_ref_subs1=mysql_num_rows($sql_ref_subs1);
							if($count_ref_subs1)
							{
							$arr_status=array("Unpaid","Unpaid","Paid");
							...
							$sql="
	SELECT invoice_amt, commission, purcheser_id, remark, level, invoice_amt,
	STATUS , paid_status, ts,date,user_level
	FROM `level_income_admin`
	WHERE income_id = '$id'
	AND level <>0 and status=0 and paid_status=0
	ORDER BY `ts` DESC";
						...
						<td class="center">
						  ...
						  <td class="center">
								<?php echo $arr_package[$row['user_level']];?>
						  </td>
						  <td class="center">$<?php echo $row['commission'];?></td>
                          <td class="center"><?php if($row['status']==2 && $row['paid_status']==2) echo "Paid"; else if($row['paid_status']==2) echo "Cancel"; else echo "Unpaid";?></td>
						</tr>
						<?php
						$ii++;
						}
						...
						?>
                        <p>
							Your Subscription is expire.<br>
                            You Need To Upgrade Your Account Subscription To Earn The Affiliate Refferal Income.<br>
                            <span ><a href="upgrade_payment_package.php" class="badge_style b_pending">Upgrade Subscription</a></span>
						</p>
                        <?php
						}
                        }
                        ...
                        <p>
							...
							You Need To Upgrade Your Account To Earn The Affiliate Refferal Income.<br>
                            <span ><a href="upgrade_account1.php" class="badge_style b_pending">Upgrade</a></span>
						</p>
                        <?php
						}
						?>
