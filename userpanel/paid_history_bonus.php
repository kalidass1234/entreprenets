...
<div id="container">
	...
	<?php
		...
		$sql_ref_subs="select * from subscription where user_id='$id' and status=0 and type=2";
		...
		$reg_date_30=date("m/d/Y",strtotime(date("Y-m-d", strtotime($row_ref_subs['subs_date'])) . " +30 days"));
		$date_30=strtotime(date("Y-m-d", strtotime($row_ref_subs['subs_date'])) . " +30 days");
		$time=time();
		?>
	<div id="content">
          ...
          <strong>Status:</strong><?php if($row_ref['user_name']){ echo "Active";}else{ echo "Inactive";}?>
			  ...
			  <div class="widget_wrap">
					<div class="widget_top">
						...
						<h6>Residual Income/Bonus History</h6>
                         <div id="widget_tab">
							<ul>
								<li><a href="#" class="active_tab">Residual/Bonus History</a></li>
								<li><a href="paid_history_ref.php">Affiliate History</a></li>
                               <?php if(showuserid($_SESSION['adid'])==12345){?>
                               <li><a href="paid_history_guru.php">Gurus History</a></li>
							   <?php }?>	
							</ul>
						</div>
				  </div>
					<div class="widget_content">
						...
						if($category_two)
						{
							//echo "select * from subscription where user_id='$id' and status=0 and type =2";
							$sql_ref_subs1=mysql_query("select * from subscription where user_id='$id' and status=0 and type =2");
						...
						$sql="SELECT invoice_amt, commission, purcheser_id, remark, level, invoice_amt,status , paid_status, ts,date,user_level,payout_date FROM `level_income_bonus`
WHERE income_id = '$id' AND level <>0 and status=2 and paid_status=2
union all
SELECT invoice_amt, commission, purcheser_id, remark, level, invoice_amt,status , paid_status, ts,date,user_level,payout_date FROM `level_income_total`
WHERE income_id = '$id' AND level <>0 and status=2 and paid_status=2
 ...
 ORDER BY `ts` DESC";
						...
						<td class="center">
                         ...
                         <td class="center">
								<?php echo date('m/d/Y',strtotime($row['payout_date']));?>
						  </td> 
						<td class="center">
						...
						<td class="center">
						  ...
						  <td class="center">
                            ...
                            <td class="center"><?php if($row['status']==2 && $row['paid_status']==2) echo "Paid"; else if($row['paid_status']==4) echo "Cancel"; else echo "Unpaid";?></td>
						</tr>
						<?php
						$ii++;
						}
                        ...
                        <p>
							...
							You Need To Upgrade Your Account To Earn The Residual Income.<br><br>
