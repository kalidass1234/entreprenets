...
function changehistorys(val){
	...
	if(val!='' && val!='<?=$_SESSION['changehistoryval'];?>'){
	  ...
	  if(con==true){
		document.location.href='my_board_bmc2.php?stage='+val;;
		return true;
	  } else {
		return false;
	  }
...
<!--                    <tr>
	...
	$boardN_lev3 = $satgeA_lev3['boardns'];
             ...
             if($boardN_lev3  != ''){
					$quer3="select * from board_status where board_owner_id='".$user3_id."' and package_id = '70.00' AND board_number= '$boardN_lev3' AND upper_id = '$user3_id' AND level='1' ORDER BY cell_no ASC";
...
<a href="" class="tooltip1">
   <img id="menu<?=$i ?>" src="../paymentimage/<?=$img?>" width="32" height="32" border="0"/>
          ...
          <tr style="color:#3399FF;">
              <?php
			   for($i=1;$i<=3;$i++)
			   {
				   ...
				   {
					   $userid=$arr3[$i-1][3];
					   $sql_stat="select * from registration where user_id='$userid' and package_amount = '70.00'";
					   $res_stat=mysql_query($sql_stat);
					   $row_stat=mysql_fetch_assoc($res_stat);
	...
	$boardN_lev3 = $satgeA_lev3['boardns'];
			   ...
			   { 
			   if($boardN_lev3 != ''){
					$quer4="select * from board_status where board_owner_id='".$user3_id."' and package_id = '70.00' AND board_number= '$boardN_lev3' AND upper_id = '$user3_id' ORDER BY cell_no ASC"; // LIMIT 6,9
	...
	$boardN_lev3 = $satgeA_lev3['boardns'];
			   ...
			   { 
			   	if($boardN_lev3 != ''){
					...
					$quer5="select * from board_status where board_owner_id='".$user3_id."' and package_id = '70.00' AND board_number= '$boardN_lev3' AND upper_id = '$user3_id' ORDER BY cell_no ASC"; // LIMIT 9,12
