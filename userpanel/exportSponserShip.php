...
$sql = "select * FROM binary_income WHERE income_id = '".$rowNomIdCount['user_id']."' AND bonus_name='3' ORDER BY id DESC";
$res = mysql_query($sql);
$totc_unpaid = 0;
$total = 0;
$content = '';
...
{
	$sqls = "select user_name from registration where 1 and user_id='".$row['purchaser_id']."'";
...
$title .= "Sr No.,Date, Purchaser Id,User Name,Package,Tds Amount, Trust Amount, Admin Amount, Commission (Rs),Final Amount (Rs),Status "."\n";
echo $title;
echo $content;
