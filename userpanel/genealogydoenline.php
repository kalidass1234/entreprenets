				...
				<div class="widget_wrap tabby">
					<div class="widget_top">
						...
						<div id="widget_tab">
							<ul>
								...
								<li><a href="totaldownline.php" class="">Total Downlines</a></li>
								<li><a href="#tab3" class="active_tab">Direct Members</a></li>
							</ul>
						</div>
					</div>
					<div class="widget_content">
						...
						<thead>
                                    	...
                                    	$query="select * from registration where ref_id='$id'";
                                    
									$q=mysql_query($query);
                                    $i=0;
                                    while($row=mysql_fetch_array($q))
									{
                                     $name1=ucfirst($row['first_name'])." ".ucfirst($row['mid_name'])." ".ucfirst($row['last_name']);
                                    $u_id=$row['user_id'];
                                    $p_id=$row['nom_id'];
                                    $r_id=$row['ref_id'];
							...
							else
							{
                             ...
                             if(_check_member_status($row['user_id'])==1)
							{
								$status='Inactive';
							}
							else if(_check_member_status($row['user_id'])==2)
							{
								$status='Cancel';
							}
							else if(_check_member_status($row['user_id'])==4)
							{
								$status='Delete';
							}
							else if($row[mem_status]=='0'){$status='Active';} else{$status='De-active';}
							 if(_check_member_status($row['user_id'])==1 || _check_member_status($row['user_id'])==2 || _check_member_status($row['user_id'])==4)
							 {
							 }
							 else
							 {
                              ?>
                                <tr>
                                  ...
                                  <td class="center"><?=$u_id;?></td>
                                  <td class="center"><?=$row['user_name'];?></td>
                                  <td class="center"><?=$name1;?></td>
                                  <td class="center"><?php echo $status;?></td>
                                  <td class="center"><?=$row['ref_id'];?></td>
                                  <td class="center"><?=showusername($row['ref_id']);?></td>
                                  <td class="center"><?=date('m-d-Y',strtotime($row['reg_date']));?></td>
                                </tr>
                               <?php
                                  $i++;
								  }
