			...
			<div class="grid_12 full_block">
				<div class="widget_wrap">
					<div class="widget_top">
						...
						<h6>Total Left PV:  <?php
    $sql_left="select SUM(invoice_bv) AS total from purchase_history where income_id='$useridd' and leg='left' and status=0";
	$res_left=mysql_query($sql_left);
	$row_left=mysql_fetch_assoc($res_left);
	if($row_left['total']){ echo $row_left['total'];}else{ echo "0";};
	?></h6>
					</div>

     <div class="widget_top">
						<span class="h_icon list_image"></span>
						<h6>Total Right PV :<?php
    $sql_left="select SUM(invoice_bv) AS total from purchase_history where income_id='$useridd' and leg='right' and status=0";
	$res_left=mysql_query($sql_left);
	$row_left=mysql_fetch_assoc($res_left);
	if($row_left['total']){ echo $row_left['total'];}else{ echo "0";};
	?></h6>
