			...
			<div class="grid_12">
				<div class="widget_wrap">
					...
					?>
						...
						if($_POST['Show']=='Search')
						{
						$query="SELECT *, sum(price*quantity) as oto FROM purchase_detail PUR inner join product_category PRO where PRO.p_cat_id=PUR.p_id  and user_id='$id' and PUR.section='marketing' and PUR.false_invoice=1 and PUR.date BETWEEN '$dt1' and '$dt2' group by invoice_no order by PUR.date desc  ";
						}
						else
						$query=" SELECT *, sum(price*quantity) as oto FROM purchase_detail PUR inner join product_category PRO where PRO.p_cat_id=PUR.p_id  and user_id='$id'  and PUR.section='marketing' and PUR.false_invoice=1 group by invoice_no order by PUR.date desc    ";
		//echo $query;				
...
//echo $query;
					...
					?>
						<tr>
							...
							<td class="sdate center">
								 <? echo date("m-d-Y", strtotime($noticia['date']));?>
							</td>
							<td class="center">
								<?=round($noticia['oto'],2);?>
							</td>
							...
							<td class="center">
								<!--<span class="badge_style b_done">--><a href="invoice.php?invoice_no=<?=$noticia[invoice_no]; ?>">View Detail</a><!--</span>-->
							</td>
