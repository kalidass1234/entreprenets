...
session_start();
//unset($_SESSION['changehistoryval']);
...
$queryMaxboard = "select board_number as boardns from board_status where user_id = '$id' and package_id = '$package_amount' AND circle='$pre_stage'";
...
//$stage = 1;
$queryMaxboard = "select max(board_number) as boardns from board_status where upper_id = '$id' and package_id = '$package_amount' ";
$resMaxboard = mysql_query($queryMaxboard);
$satgeA = mysql_fetch_array($resMaxboard);
...
$str = "select * from board_status where upper_id='$id' and package_id = '$package_amount' AND upper_id = '$id' AND board_number= '$boardN' AND level='1' ORDER BY cell_no ASC ";
$res = mysql_query($str);
...
$quer2 = "select * from board_status where upper_id='$id' and package_id = '$package_amount' AND board_number= '$boardN' AND upper_id = '$id'  AND level='1' ORDER BY cell_no ASC";
...
$stage_val = (($stage-1)*3);
$quer2 = "select * from board_status where upper_id='$id' and package_id = '$package_amount' AND board_number= '$boardN' AND upper_id = '$id' AND level='1' AND level='1' ORDER BY cell_no ASC LIMIT $stage_val , 3";
...
if (con == true) {
document.location.href = 'my_board_bmc1.php?stage=' + val;
;
return true;
} else {
return false;
...
if ($count_subs1) {
?>
                <td><? echo $useridd . "<br>" . $name2 . "<br>" . "$" . $row_stat['package_amount']; ?><br><?php if ($row_stat[mem_status]) {
...
$lev2_user_id = '';
 ...
 echo "level2user".$use_lev2[0];


if(isset($use_lev2[0])){
$lev2_user_id = $use_lev2[0];
}
...
//$quer3 = "select * from registration where nom_id='" . $nom . "' ORDER BY id ASC ";
$quer3 = "select * from registration where user_id='$lev2_user_id' and package_amount = '$package_amount'";
...
if ($i <= $ro3 && $lev2_user_id != '') {
$d = $arr3[$i - 1][1];

$str3 = "select * from registration where user_id='$lev2_user_id' and package_amount = '$package_amount'";
/*echo "level 3";
var_dump($str3);*/
...
for ($i = 1; $i <= 3; $i++) {
$nom = $arr2[0][3];
$ro3 = mysql_num_rows(mysql_query("select * from registration where user_id='$lev2_user_id' and package_amount = '$package_amount'"));
...
if ($i <= $ro3) {
$userid = $arr3[$i - 1][3];
//$sql_stat = "select * from registration where (user_name='$userid' OR user_id='$userid') and package_amount = '$package_amount'";
$sql_stat = "select * from registration where user_id='$lev2_user_id' and package_amount = '$package_amount'";
$res_stat = mysql_query($sql_stat);
$row_stat = mysql_fetch_assoc($res_stat);
...
if ($count_subs1) {
?>
    <td><? echo $row_stat['user_id'] . "<br>" . $row_stat['user_name'] . "<br>" . "$" . $row_stat['package_amount']; ?><br><?php if ($row_stat[mem_status]) {
echo "<font color='red'>Inactive</font>";
} ?></td>
<?
} else {
...
$quer4 = "select * from registration where user_id='$lev2_user_id' and package_amount = '$package_amount'";
...
if ($i <= $ro4 && $lev2_user_id != '') {
$d = $arr4[$i - 1][1];
$str4 = "select * from registration where user_id='$lev2_user_id' and package_amount = '$package_amount'";
...
//print_r($arr3[$i-1]);
$nom = $arr2[1][3];
$ro4 = mysql_num_rows(mysql_query("select * from registration where user_id='$lev2_user_id' and package_amount = '$package_amount'"));
...
if ($nom != '') {
    ?>
    <td><? echo $x4['user_id'] . "<br>" . $name4 . "<br>" . "$" . $x4['package_amount']; ?><br><?php if ($x4[mem_status]) {
        echo "<font color='red'>Inactive</font>";
    } ?></td>
<?
} else {
...
$lev2_user_id = '';
if(isset($use_lev2[2])){
$lev2_user_id = $use_lev2[2];
}
for ($i = 1; $i <= 3; $i++) {
$nom = $arr2[2][3];
$quer5 = "select * from registration where user_id='$lev2_user_id' and package_amount = '$package_amount'";
...
if ($i <= $ro5 && $lev2_user_id != '') {
$d = $arr5[$i - 1][1];
$str5 = "select * from registration where user_id='$lev2_user_id' and package_amount = '$package_amount'";
