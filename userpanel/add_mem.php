                  ...
                  <form id="forms1" name="forms1"  method="post" class="form_container left_label">
							...
							<li>
								...
								<div class="form_grid_12">
									...
									<div class="form_input">
									<input type="text" name="uname" id="name"  onBlur="checksponser('user_name1',this.value);"><input type="hidden" name="id" value=""  />
                                    <label for="user_name1_error" id="user_name1_error" class="error" style="display:none">Please enter username</label>
									</div>
								</div>
								</li>
								<li>
...
if(isset($_POST['Show']))
{
	...
	$num_rows=mysql_num_rows($data);
				...
				if($num_rows)
				{
						...
						<?php
						$idq=$arr['user_id'];
						$sql_check="select * from benificiery where member_id='$idq' and user_id='$id' and type='$_REQUEST[type]'";
						$res_check=mysql_query($sql_check);
						$count_check=mysql_num_rows($res_check);
						if($count_check>0)
						{
						?>
    					<td class="center"><?php echo $msg="Username Already Added";?></td>
						<?php
                        }
                        else
                        {	
                        ?>
						<td class="center"><a href="add_beni_func.php?dtl=<?php echo $arr['user_id']; ?>&amp;type=<?php echo $_REQUEST['type'];?>&return_url=fund_trans.php" class="head">Add</a></td>
                        <?php
						}
						?>
						</tr>
...
<script>
function checksponser(id,value)
{
	document.getElementById(id+'_error').style.display='inline';
	//jQuery('#'+id+'_error').show();
	//alert('ajax_user.php?action='+id+'&user='+value);
	jQuery.ajax({
					url: '../ajax_user.php?action='+id+'&user='+value,
					type: 'GET',
					dataType: 'html',
					beforeSend: function() {
						jQuery('#'+id+'_error').html('<img src="../images/ajax-loader.gif" />');
					},
					success: function(data, textStatus, xhr) {
						//alert(data);
							jQuery('#'+id+'_error').html(data);
					},
					error: function(xhr, textStatus, errorThrown) {
						jQuery('#'+id+'_error').html(textStatus);
					}
				});
	//setTimeout( function() {$('#'+id+' .contentarea').hide();}, 2000);						
}
