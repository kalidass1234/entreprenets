...
function shownext1(path)
{
	...
	if(document.getElementById('other_category').checked==true)
	{
		var other_category=document.getElementById('other_category').value;
		post_to_url('total_amount_one.php', {'pin':pin,'return_page':'member-secure_two_new.php','other_category':other_category,'other_duration':'12'});
	}
	else
	{
		var other_category='';
		post_to_url(path, {'pin':pin,'return_page':'member-secure_two_new.php','other_category':other_category,'other_duration':'12'});
	}
...
var inputtxt=document.getElementById('card_no');
	...
	{
		var other_category=document.getElementById('other_category').value;
		post_to_url('total_amount_one.php', {'x_first_name':x_first_name,'x_last_name':x_last_name,'exp_year':exp_year,'exp_month':exp_month,'card_type':card_type,'card_no':card_no,'cvv':cvv,'x_address':x_address,'x_city':x_city,'x_state':x_state,'x_zip':x_zip,'x_mobile':x_mobile,'x_email':x_email,'other_category':other_category,'other_duration':'12'});
	}
	else
	{
		var other_category='';
		post_to_url(path, {'x_first_name':x_first_name,'x_last_name':x_last_name,'exp_year':exp_year,'exp_month':exp_month,'card_type':card_type,'card_no':card_no,'cvv':cvv,'x_address':x_address,'x_city':x_city,'x_state':x_state,'x_zip':x_zip,'x_mobile':x_mobile,'x_email':x_email,'other_category':other_category,'other_duration':'12'});
	}
...
Membership initial registration fee of $29.99 for access to the full range of membership services provided by the Vision Team Network; including a virtual office, payout, records information services, visa pay-card and 24-hour customer service support. After 30 days, a reoccurring monthly membership fee of $29.99 and $54.99 for your online virtual office will be required to maintain and access the full membership services provided by the Vision Team Network online services. 
<?php
}
?>
<?php if($_SESSION['duration']==3){?>
Membership initial registration fee of $29.99 for access to the full range of membership services provided by the Vision Team Network; including a virtual office, payout, records information services, visa pay-card and 24-hour customer service support. After 90 days, a reoccurring monthly membership and virtual office service fee of $254.94 will be required to maintain and access the full membership services provided by the Vision Team Network online services.
<?php }?>
<?php if($_SESSION['duration']==6){?>
Membership initial registration fee of $29.99 for access to the full range of membership services provided by the Vision Team Network; including a virtual office, payout, records information services, visa pay-card and 24-hour customer service support. After 6 months, a reoccurring monthly membership and virtual office service fee of $509.88 will be required to maintain and access the full membership services provided by the Vision Team Network online services.
<?php }?>
<?php if($_SESSION['duration']==12){?>
Membership initial registration fee of $29.99 for access to the full range of membership services provided by the Vision Team Network; including a virtual office, payout, records information services, visa pay-card and 24-hour customer service support. A reoccurring monthly membership fee of $29.99 and $54.99 for your online virtual office will be required to maintain and access the full membership services provided by the Vision Team Network online services.
<?php }?>
</p>

<p style="margin-left:100px; margin-right:100px;"> 
<strong>
<?php if($_SESSION['duration']==1){?>The price for membership is $29.99 now and then $84.98 per Month for 12 more payments. All Members will be charged  a Administration Fee. Membership expires after 1 year.<?php }?>
...
<?php if($_SESSION['duration']==12){?>The price for membership is $29.99 now and then $1,019.76 every 12 Months. All Members will be charged a Administration Fee. Membership expires after 1 year.<?php }?>
</strong>.  </p>
</td>
    </tr>
...
NOT LATER THAN MIDNIGHT of the third business day following the date set forth above
    ...
    function showpaymenttotal(drt,cat)
	{
		...
		if(cat==3 && document.getElementById('other_category').checked==true)
		{
		...
		if(document.getElementById('other_category').checked==false)
		{
...
background: #fff;
color: #777;">
       <input type="checkbox" id="other_category" name="other_category" value="3" onClick="showpaymenttotal(<?php echo $_SESSION['duration'];?>,3)" />&nbsp;&nbsp; <label for="tos">You Can Select Affiliate Refferal Registration(Membership Subscription $150 per Year)</label>
      </td>
    </tr>
    <tr >
...
color: #777;">
    ...
    <tr>
      ...
      <td><input type="text" name="x_first_name" id="x_first_name" style="width:50%" value="<?php echo $_SESSION['fname'];?>" /></td>
    ...
    <td><input type="text" name="x_address" id="x_address" style="width:50%" value="<?php echo $_SESSION['address1'];?>" /></td>
    ...
    <td><input type="text" name="x_email" id="x_email" value="<?php echo $_SESSION['email'];?>" style="width:50%" /></td>
