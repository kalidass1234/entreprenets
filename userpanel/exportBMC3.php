...
header("Content-Disposition: attachment; filename=exportBMC2.csv");
...
$sql = "select * from level_income where 1 AND income_id='$id' AND package_id='12'  order by l_id desc ";
$res = mysql_query($sql);
$totc_unpaid = 0;
$total = 0;
$content = '';
...
{
	$select = mysql_fetch_array(mysql_query("SELECT * FROM registration WHERE 1 and user_id='".$row['purchaser_id']."'"));
	//$res_catb=mysql_fetch_array(mysql_query("select SUM(commission) as commissions from level_income where package_id='13' AND income_id!='56789' AND income_id='$id' "));
