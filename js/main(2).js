...
similarproducts.gameHour  =
{
	...
	processServerData: function(data) // This function is called by the "utils.serverMessagesRouter" func via the "fn" param send by the server layer
	{
        ...
        if  (similarproducts.utilities.getActiveUnits() != ''){
            similarproducts.sfdebugger.log('<b>No need to show gameHour unit - [' + similarproducts.utilities.getActiveUnits() + '] Showed</b>');
            this.reportAction({action: 'gameHourNoNeedToShow',dscr: similarproducts.utilities.getActiveUnits()});
            return;
        }
...
//		similarproducts.info.pi("-9999" + similarproducts.b.xdMsgDelimiter + spsupport.p.initialSess);
        ...
        similarproducts.info.setUnitConfiguration('gameHour',
        {
